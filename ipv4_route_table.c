 ...
 */
ipv4_route_t * ipv4_route_create
( ipv4_addr_t subnet, ipv4_addr_t mask, char* iface, ipv4_addr_t gw, unsigned int adist )
{
...
void ipv4_route_print ( ipv4_route_t * route )
{
  if (route != NULL) {
    ...
    unsigned int adist = route->admin_dist;
...
int ipv4_route_table_read ( char * filename, ipv4_route_table_t * table )
{
  ...
  while ((! feof(routes_file)) && (err==0)) {
    ...
    if (err != 4) {
      ...
      unsigned int admin_dist = ADMIN_DIST_STATIC;
      if (memcmp(gateway, IPv4_ZERO_ADDR, IPv4_ADDR_SIZE) == 0) {
	  admin_dist = ADMIN_DIST_DIRECT;
      }

      /* Create new route & add it to Route Table */
      ipv4_route_t * new_route = 
        ipv4_route_create(subnet, mask, iface_name, gateway, admin_dist);
 ...
 *   la salida indicada.
 */
int ipv4_route_table_output ( FILE * out, ipv4_route_table_t * table, int adist )
{
  int err;
  ...
  if (adist == 0) {
    ...
    err = fprintf
      (out, "# SubnetAddr  \tSubnetMask    \tIface  \tGateway       \tAdmin\n");
  }
  if (err < 0) {
    return -1;
  }
  ...
  for (i=0; i<IPv4_ROUTE_TABLE_SIZE; i++) {
    ...
    if (route_i != NULL) {
      ...
      admin_dist = route_i->admin_dist;

      if (adist == 0) {
	err = fprintf(out, "%-15s\t%-15s\t%s\t%-15s\n",
		      subnet_str, mask_str, ifname, gw_str);
      } else {
	err = fprintf(out, "%-15s\t%-15s\t%s\t%-15s\t%d\n",
		      subnet_str, mask_str, ifname, gw_str, admin_dist);
      }
      if (err < 0) {
        return -1;
      }
...
int ipv4_route_table_write ( ipv4_route_table_t * table, char * filename )
{
  ...
  if (table != NULL) {
    num_routes = ipv4_route_table_output (routes_file, table, 0);
    if (num_routes == -1) {
...
{
  if (table != NULL) {
    ipv4_route_table_output (stdout, table, 1);
  }
}
